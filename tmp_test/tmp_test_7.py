...
# (bug!) 这个prompt有问题，MessagesPlaceholder打印出来是 {{messages}}，而不是 {messages}
# prompt = hub.pull("wfh/react-agent-executor")

# (fixed!) 因此，在这里进行手写替换，亲测有效
...
# 调用代理执行器
# tmp_response = agent_executor.invoke({"messages": [("user", "谁是美国公开赛的获胜者")]})
...
# 使用指定的提示词模板，创建一个计划生成器，使用OpenAI的ChatGPT-4o模型
...
# 类中有一个属性`action`，类型为`Union[Response, Plan]`，表示可以是Response或Plan类型
# action属性有描述
class Act(BaseModel):
    """要执行的行为"""
...
replanner_prompt = ChatPromptTemplate.from_template(
    ...
    相应地更新你的计划。
    如果不需要更多步骤并且可以返回给用户，那么就这样响应。
    如果需要，填写计划。
    只添加仍然需要完成的步骤。不要返回已完成的步骤作为计划的一部分。
    """
)

# 使用指定的提示词模板，创建一个重新计划生成器，使用OpenAI的ChatGPT-4o模型
...
async def main():
    ...
    async def execute_step(state: PlanExecute):
        ...
        # agent_response= await agent_executor.ainvoke({"messages": [HumanMessage(content=task_formatted)]})
    ...
    with open("../dataset/tmp_test_7.png", "wb") as f:
        f.write(graph_png)
